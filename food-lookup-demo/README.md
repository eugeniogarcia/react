...
This request is made to `localhost:3000`, the Webpack dev server. __Webpack will infer that this request is actually intended for our API server__. We specify in `package.json` that we would like Webpack to proxy API requests to `localhost:3001`:
